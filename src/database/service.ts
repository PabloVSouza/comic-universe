...
export class DatabaseService {
  ...
  async getChaptersByComicId(comicId: number): Promise<Chapter[]> {
    return await this.db.select().from(chapters).where(eq(chapters.comicId, comicId)).orderBy(asc(chapters.number));
  }
  ...
  async updateChapter(id: number, chapter: Partial<NewChapter>): Promise<Chapter | undefined> {
    const result = await this.db.update(chapters).set(chapter).where(eq(chapters.id, id)).returning();
    return result[0];
  }
  ...
  async getReadProgressByComic(comicId: number, userId: number): Promise<ReadProgress[]> {
    return await this.db.select().from(readProgress).where(
      and(eq(readProgress.comicId, comicId), eq(readProgress.userId, userId))
    );
  }

  async getReadProgressByChapter(chapterId: number, userId: number): Promise<ReadProgress | undefined> {
    const result = await this.db.select().from(readProgress).where(
      and(eq(readProgress.chapterId, chapterId), eq(readProgress.userId, userId))
    ).limit(1);
    return result[0];
  }
  ...
  async updateReadProgress(id: number, progress: Partial<NewReadProgress>): Promise<ReadProgress | undefined> {
    const result = await this.db.update(readProgress).set(progress).where(eq(readProgress.id, id)).returning();
    return result[0];
  }
  ...
  async getEnabledPlugins(): Promise<Plugin[]> {
    return await this.db.select().from(plugins).where(eq(plugins.enabled, true)).orderBy(asc(plugins.name));
  }
  ...
  // Complex queries
  async getComicWithChapters(comicId: number): Promise<{ comic: Comic; chapters: Chapter[] } | undefined> {
  ...
  async getComicWithProgress(comicId: number, userId: number): Promise<{
    comic: Comic;
    chapters: Chapter[];
