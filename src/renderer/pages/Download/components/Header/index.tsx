...
const DownloadChapterHeader = (): JSX.Element => {
    ...
    <div className={style.header}>
      <Container className={style.container}>
        <div className={style.texts}>
          ...
          {!!comic.publisher && <p className={style.publisher}>{comic.publisher}</p>}
          {!!comic.genres && (
            <p className={style.genre}>{!!comic.genres && comic.genres.join(', ')}</p>
          )}
          {!!comic.status && <p className={style.status}>{comic.status}</p>}
          <div className={style.synopsis}>{ReactHtmlParser(comic.synopsis)}</div>
